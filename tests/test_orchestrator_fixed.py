"""Tests for the orchestrator coordinator - Fixed version."""
...
class TestResearchCoordinator:
    ...
    async def test_initialize_success(self, coordinator):
        ...
        with patch('app.database.connection.db_manager') as mock_db_manager, \
             patch('app.llm.client.llm_client') as mock_llm_client:
            
            mock_llm_client.initialize = AsyncMock(return_value=True)
            mock_db_manager.initialize = AsyncMock(return_value=MagicMock(success=True))
