...
class TestResearchCoordinator:
    ...
    async def test_conduct_research_success(self, coordinator, sample_query, sample_report, sample_feedback):
        ...
        with patch('app.database.connection.db_manager') as mock_db_manager, \
            ...
            mock_db_manager.create_task.return_value = MagicMock(success=True, data={"id": "task123"})
    ...
    async def test_conduct_initial_research(self, coordinator, sample_query, sample_report):
        ...
        with patch.object(coordinator.researcher, 'process') as mock_research_process, \
             patch.object(coordinator.db_manager, 'save_report') as mock_save_report, \
             patch.object(coordinator.db_manager, 'log_agent_message') as mock_log_message:
    ...
    async def test_self_correction_loop_high_quality(self, coordinator, sample_query, sample_report, sample_feedback):
        ...
        with patch.object(coordinator, '_critique_report') as mock_critique, \
             patch.object(coordinator, '_revise_report') as mock_revise, \
             patch.object(coordinator.db_manager, 'update_task') as mock_update, \
             patch.object(coordinator.db_manager, 'log_agent_message') as mock_log:
    ...
    async def test_self_correction_loop_with_revisions(self, coordinator, sample_query, sample_report):
        ...
        with patch.object(coordinator, '_critique_report') as mock_critique, \
            ...
            mock_critique.side_effect = [low_quality_feedback, sample_feedback]
    ...
    async def test_critique_report(self, coordinator, sample_query, sample_report, sample_feedback):
        ...
        with patch.object(coordinator.critic, 'process') as mock_critique_process, \
             patch.object(coordinator.db_manager, 'save_feedback') as mock_save_feedback, \
             patch.object(coordinator.db_manager, 'log_agent_message') as mock_log:
    ...
    async def test_revise_report(self, coordinator, sample_query, sample_report, sample_feedback):
        ...
        with patch.object(coordinator.reviser, 'process') as mock_revise_process, \
             patch.object(coordinator.db_manager, 'log_agent_message') as mock_log:
    ...
    async def test_get_task_status(self, coordinator):
        ...
        with patch.object(coordinator.db_manager, 'get_task') as mock_get_task:
            mock_get_task.return_value = MagicMock(success=True, data=mock_task_data)
